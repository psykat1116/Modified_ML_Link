...
def combine_scores(p_ml, p_temp, alpha_param, beta_param=None):
    threshold = 0.5

    ml_pos   = p_ml   >= threshold   # ML-Link says edge exists
    temp_pos = p_temp >= threshold   # Temporal says edge exists

    # Both agree edge exists → return higher of the two
    # Both agree no edge    → return lower of the two  
    # Disagree              → return score of more confident one

    conf_ml   = torch.abs(p_ml   - threshold)
    conf_temp = torch.abs(p_temp - threshold)

    agree_pos  = ml_pos  & temp_pos    # both say yes
    agree_neg  = ~ml_pos & ~temp_pos   # both say no

    p_out = torch.where(agree_pos,  torch.maximum(p_ml, p_temp),   # both yes → max
            torch.where(agree_neg,  torch.minimum(p_ml, p_temp),   # both no  → min
            torch.where(conf_ml >= conf_temp, p_ml, p_temp)))       # disagree → more confident
