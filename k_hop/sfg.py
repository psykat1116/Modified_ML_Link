...
class MLStruct(torch.nn.Module):
    ...
    def forward(self, g, g_edges, edge_w=False, g_khop=None):
        """
        g_khop : optional list of DGL graphs whose adjacency represents the
                 up-to-k-hop neighbourhood.  When provided, both the node
                 structural-feature aggregation and the overlap computation use
                 the k-hop adjacency instead of the 1-hop adjacency.
                 When None the original 1-hop behaviour is preserved.
        """
        out_structs = []
        node_struct_feats = []
        for l_id in range(self.n_layers):
            ...
            # ── choose 1-hop or k-hop graph for structural feature extraction ──
            src_g = g_khop[l_id] if g_khop is not None else g[l_id]

            rows, cols, eids = src_g.edges(order='srcdst', form='all')

            edge_weights = src_g.edata['w'][eids].to(device)
            edge_weight_A = f_edge(edge_weights.unsqueeze(-1))
            node_struct_feat = scatter_add(edge_weight_A, cols, dim=0, dim_size=src_g.number_of_nodes())
            ...
            # ── adjacency used for overlap score computation ───────────────────
            A = src_g.adj_external(scipy_fmt="csr")
            if edge_w and g_khop is None:   # edge_w only meaningful for 1-hop
...
class MAA(torch.nn.Module):
    ...
    def forward(self, g, g_edges, out_structs, node_struct_feats, p, edge_w=False, g_khop=None):
        """
        g_khop : optional list of k-hop DGL graphs (one per layer).
                 When provided, MAAN uses the k-hop adjacency of each layer
                 instead of the 1-hop adjacency.
        """
        device = g[0].device
        out_structs2 = []
        ...
        for lid1 in range(self.n_layers):
            ...
            # ── adjacency for target layer lid1 ───────────────────────────────
            src_g1 = g_khop[lid1] if g_khop is not None else g[lid1]
            A = src_g1.adj_external(scipy_fmt="csr")
            if edge_w and g_khop is None:
            ...
            for i, lid2 in enumerate(pol):
                ...
                if lid1 != lid2:
                    ...
                    # ── adjacency for context layer lid2 ──────────────────────
                    src_g2 = g_khop[lid2] if g_khop is not None else g[lid2]
                    A2 = src_g2.adj_external(scipy_fmt="csr")
                    if edge_w and g_khop is None:
...
class Glob(torch.nn.Module):
    ...
    def forward(self, g, g_edges, out_structs, node_struct_feats, p, edge_w=False, g_khop=None):
        """
        g_khop : optional list of k-hop DGL graphs.
                 For OAN (Glob), the merged adjacency is replaced by the
                 union of the k-hop adjacencies of the two layers:
                     A_oan_k = binarize(khop[lid1] + khop[lid2])
                 This is the natural k-hop extension of OAN.
        """
        device = g[0].device
        out_structs2 = []
        cache = {}
            ...
            for i, lid2 in enumerate(pol):
                ...
                if lid1 != lid2:
                    if (lid1, lid2) not in cache:
                        if g_khop is not None:
                            # ── k-hop OAN: union of k-hop adjacencies ─────────
                            A1k = g_khop[lid1].adj_external(scipy_fmt="csr")
                            A2k = g_khop[lid2].adj_external(scipy_fmt="csr")
                            A = A1k + A2k
                            A.data = np.ones_like(A.data)   # binarize union
                            A.setdiag(0)
                            A.eliminate_zeros()
                        else:
                            # ── original 1-hop OAN: merge graphs ─────────────
                            ...
                            A = g_merged.adj_external(scipy_fmt="csr")
                            if edge_w:
