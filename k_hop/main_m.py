...
class Mm(nn.Module):
    ...
    def forward(self, g_supra, g, p, g_edges, features, predictor,
                inter_layer=None, g_khop=None):
        """
        g_khop : optional list of DGL graphs (one per layer) whose adjacency
                 represents the up-to-k-hop neighbourhood.
                 Passed unchanged to the structural components (MLStruct,
                 MAA, Glob).  The GNN-NE component always uses the original
                 1-hop supra-adjacency (g_supra) regardless of g_khop.
                 When None, the original 1-hop behaviour is preserved.
        """
        if self.struct:
            out_structs1, node_struct_feats = self.struct_model(
                g, g_edges, edge_w=self.edge_w, g_khop=g_khop
            )
            out_structs2 = []
            if inter_layer:
                for mode in inter_layer:
                    if mode == C.MAAN:
                        tacl = self.tacl(
                            g, g_edges, out_structs1, node_struct_feats, p,
                            edge_w=self.edge_w, g_khop=g_khop
                        )
        ...
        if self.gnn:
            # GNN always operates on the 1-hop supra-adjacency
